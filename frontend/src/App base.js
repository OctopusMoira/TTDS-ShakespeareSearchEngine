...
export default class App extends Component {
  ...
  toEnter = e => {
    ...
    if (e.key === "Enter") {
      if (query.length) {
        ...
        this.setState({showalert: true, warningTitle: "Invalid", warning: "Query shall not be empty.", severity: "warning"})
      }
    }
  };
  ...
  querySearch = async query => {
    ...
    // add ...
    this.setState({showalert: true, warningTitle: "Loading", warning: "Please wait patiently.", severity: "success"})
  ...
  RegularEx = async query => {
    ...
    if(response.data.result.length>=10){
      this.setState({sugWords:response.data.result, suggestions:[], dohide: true,sugTotal:response.data.result.length});
    }
    else{
